...
{
    public abstract class FilterBindingList<T> : BindingList<T>, IBindingListView
    {
        protected abstract FilterBindingList<T> NewInstance();
        protected FilterBindingList() { }
        protected abstract IEnumerable GetOriginList();
        protected abstract int CountOrigin();
        protected abstract void AddOrigin(T o);
        protected abstract void RemoveOrigin(int index);
        ...
        FilterBindingList<T> unsortedItems;
        bool isSortedValue;
        ...
        protected override void RemoveSortCore()
        {
            ...
            if (unsortedItems != null && CountOrigin() > 0)
            {
                this.Clear();
                if (Filter != null)
                {
                ...
                {
                    foreach (var item in GetOriginList())
                    {
                        this.Add((T)item);
                    }
                }
